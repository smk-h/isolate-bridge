...
同步命令里出现的路径，来源分两种：**本地侧靠 MCP 占位符替换**，**服务器侧靠工具内部解析**：
...
- MCP **只做占位符替换、不做路径解析**：`{local_root}` / `{src}` / `{dst}` 替换成字面字符串，其余原样交给 spawn 的工具。
- **服务器侧永远只写相对目录**（`outbound/`、`inbound`），不带 `nfs/vm_share/` 前缀：绝对路径由工具按自己的服务器根 `MSGFERRY_SYNC_MOCK_SERVER` 解析。
