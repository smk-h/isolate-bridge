...
Worker 运行在**外网 Windows 宿主机**、MCP 运行在**内网 Linux 虚拟机**，两者看到的是**同一个物理 HGFS 共享目录**，但操作系统不同、路径写法不同，Worker 用命令行 `--hgfs-root`、MCP 用环境变量 `MSGFERRY_LOCAL_ROOT` 各填各侧的系统路径（MCP 侧已收敛为纯环境变量注入，不再支持命令行参数）：
...
- **MCP 侧**：`.mcp.json` 的 `MSGFERRY_LOCAL_ROOT` 环境变量写 **Linux 路径**（如 `/mnt/hgfs/sharedir/vm_share`），也可用 `$HOME/.msgferry/vm_share` / `~/.msgferry/vm_share` 写法，MCP Server 启动时用 `os.homedir()` 自动展开为家目录绝对路径；**若该目录不存在，MCP 启动时会自动 `mkdir` 创建**。MCP 不读 `config/worker.yaml`，无其他路径配置；MCP Server 的全部配置（`MSGFERRY_*` / `LOG_SAVE` / `LOG_DIR`）均由环境变量注入，环境变量未定义时走内置默认值。
