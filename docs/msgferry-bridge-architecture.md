...
- **命令白/黑名单**：外网 Worker 维护命令白名单前缀（如 `docker`、`kubectl`、`systemctl status`、`journalctl`、`cat`、`ls`、`tail`）与高危黑名单（`rm -rf /`、`dd`、`mkfs`、`:(){:|:&};:`）。
- **参数校验**：命令经 `shell-quote` 解析后做参数白名单校验，未命中策略的命令直接进入 `failed` 队列，`error_msg=blocked_by_policy`。
