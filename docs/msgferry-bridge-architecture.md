...
Worker 运行在**外网 Windows 宿主机**、MCP 运行在**内网 Linux 虚拟机**，两者看到的是**同一个物理 HGFS 共享目录**，但操作系统不同、路径写法不同，Worker 用命令行 `--hgfs-root`、MCP 用环境变量 `MSGFERRY_HGFS_ROOT` 各填各侧的系统路径（MCP 侧已收敛为纯环境变量注入，不再支持命令行参数）：
