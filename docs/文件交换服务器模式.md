...
- `MSGFERRY_HGFS_ROOT`：填**内网本地镜像目录**（如 `/mnt/hgfs/sharedir/vm_share`），不再是共享挂载点，而是内网这一侧的 `outbound/` / `inbound/` 工作目录；
- `MSGFERRY_SYNC_PUSH_CMD`：**必填**。上传方向模板命令，必须含 `{src}`（本地单任务文件）与 `{dst}`（服务器 `outbound/` 目录）两个占位符，MCP 每次 push 前做纯字符串替换；
...
msgferry-worker --hgfs-root E:\SyncServer\vm_shared

:: 需要业务日志落盘时，追加日志两个命令行参数
msgferry-worker --hgfs-root E:\SyncServer\vm_shared --log-save 1 --log-dir logs\worker
```

关键点：

- `--hgfs-root` 填**文件交换服务器在 Windows 侧的挂载目录**（如上例 `E:\SyncServer\vm_shared`），Worker 在 exchange 模式下扫 `outbound/` 领任务、结果写 `inbound/`；
