...
> MsgFerry 在「更严格的隔离网络」下的部署形态：不使用共享目录，而是通过一台**文件交换服务器**（file_transfer / file_rsync 等自定义同步命令）完成内网与外网之间的文件摆渡。

---

## 一、为什么需要交换服务器模式

有的内网和外网数据交换更加严格，**根本不支持共享目录（HGFS/挂载盘）**，但会有一台专门用于文件交换的服务器。典型同步命令：
...
```
内网本地目录 hgfs_root/            文件交换服务器 vm_shared/（Worker 直接读写）
├─ outbound/                       ├─ outbound/
...
**约定**：内网本地 `inbound/` 只做镜像，**禁止本地往里写任何文件**——`-g` 整目录覆盖会把它冲掉。
...
- push 命令里 `{dst}` 在替换时统一为 `outbound/`（相对服务器根目录，MCP 不校验）；
...
### 4.2 任务执行（submit_ssh_task，仍为同步阻塞）

```
Claude 调 submit_ssh_task(cmd)
  └─ MCP: syncPull（拉回心跳，判定 worker 在线）
