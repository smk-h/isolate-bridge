...
| 环境变量 | 说明 |
|---|---|
| `MSGFERRY_SYNC_PUSH_CMD` | push 模板命令，必须含 `{src}`（本地任务文件相对路径，如 `outbound/<id>.json`）与 `{dst}`（服务器 outbound/ 相对目录）占位符，目录前缀由模板承担 |
...
模板前缀方案下，**目录前缀（如 `vm_share/`、`nfs/vm_share/`）完全由用户在同步命令模板里写**，`{src}` / `{dst}` 只替换成相对路径，换目录只需改一行模板：

```bash
# 上传方向：单文件上传到服务器 outbound/（{src} 替换为 outbound/<id>.json，{dst} 替换为 outbound/）
MSGFERRY_SYNC_PUSH_CMD='file_transfer -pd vm_share/{src} nfs/vm_share/{dst}'
# 或自定义同步工具（file_rsync 同样适用）
MSGFERRY_SYNC_PUSH_CMD='file_rsync -av vm_share/{src} nfs/vm_share/{dst}'

# 拉取方向：整目录拉回服务器 inbound/ 到本地镜像（静态字符串，不替换占位符，源带前缀）
MSGFERRY_SYNC_PULL_CMD='file_transfer -g nfs/vm_share/inbound /root/.msgferry/vm_share/inbound'
```

### 3. 模板语法规则

- 占位符替换用纯字符串 `replaceAll`，**不做 shell 解析**；
- push 命令里 `{src}` 在每次 push 前替换为**本地任务文件相对路径** `outbound/<task_id>.json`（目录前缀如 `vm_share/` 由模板承担）；
  ...
  "mcpServers": {
    "msgferry-bridge-exchange": {
      ...
      "env": {
        ...
        "MSGFERRY_SYNC_PUSH_CMD": "file_transfer -pd vm_share/{src} nfs/vm_share/{dst}",
        "MSGFERRY_SYNC_PULL_CMD": "file_transfer -g nfs/vm_share/inbound $HOME/.msgferry/vm_share/inbound",
...
- `MSGFERRY_SYNC_PUSH_CMD`：**必填**。上传方向模板命令，必须含 `{src}`（替换为本地任务文件相对路径 `outbound/<id>.json`）与 `{dst}`（替换为服务器 `outbound/` 相对目录）两个占位符；**目录前缀（如 `vm_share/`、`nfs/vm_share/`）由模板承担**，换目录只改这一行；
- `MSGFERRY_SYNC_PULL_CMD`：**必填**。拉取方向静态命令，整目录把服务器 `inbound/` 拉回本地镜像，不替换占位符；
- `MSGFERRY_SYNC_TIMEOUT_MS`：单次同步命令超时（默认 `30000`）；
- `MSGFERRY_SYNC_RETRIES`：同步失败退避重试次数（默认 `3`，退避 1s / 2s / 4s）；
...
- `-pd vm_share/{src} nfs/vm_share/{dst}`：单文件上传，src 剥离模板前缀 `vm_share/` 后相对内网本地根 `test/temp` 解析，dst 带前缀 `nfs/vm_share/` 相对服务器根 `test/temp_server` 解析，落点 `test/temp_server/nfs/vm_share/outbound/`；
- `-g nfs/vm_share/inbound <本地镜像>`：整目录拉回并完全替换本地目录，源带前缀相对服务器根解析；
- 服务器根由环境变量 `MSGFERRY_SYNC_MOCK_SERVER` 指定（默认 `test/temp_server`）；
- 内网本地根由 `MSGFERRY_SYNC_MOCK_LOCAL` 指定（默认内网 `test/temp`），模板 src 前缀由 `MSGFERRY_SYNC_MOCK_LOCAL_PREFIX` 指定（默认 `vm_share/`）；
