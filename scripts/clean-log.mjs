...
// 清理 MsgFerry 业务日志（mcp-server / worker 共用的 logs/mcp-server 目录）
...
const logDir = logDirRaw
  ? isAbsolute(logDirRaw)
    ? logDirRaw
    : join(hgfsRoot, logDirRaw)
  : join(hgfsRoot, 'logs', 'mcp-server');
