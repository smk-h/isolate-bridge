...
export interface McpServerConfig {
  local_root: string;                   // 内网本地根目录绝对路径（shared 模式为 HGFS 共享根，exchange 模式为本地任务目录）
  ...
  sync_push_cmd?: string;               // push 模板命令（含 {src}/{dst}/{local_root} 占位符），exchange 模式必填
 ...
 * @param env - 进程环境变量
 * @param key - 环境变量名（如 'MSGFERRY_LOCAL_ROOT'）
 ...
 * - 仅读取 MSGFERRY_LOCAL_ROOT / MSGFERRY_MAX_WAIT_MS /
...
export function parseConfig(_argv: string[], env: NodeJS.ProcessEnv): McpServerConfig {
  // MSGFERRY_LOCAL_ROOT 支持 `$HOME`/`~` 占位符，启动时自动展开为家目录绝对路径
  const localRoot = expandHomeDir(getEnv(env, 'MSGFERRY_LOCAL_ROOT') ?? '');
 ...
 * @throws {Error} local_root 不存在或不可读写时抛错
 */
export function validateConfig(config: McpServerConfig): void {
  if (!config.local_root) {
    throw new Error('local_root is required');
  }
  // 目录不存在时自动创建（幂等，递归创建父级），避免手动 mkdir
  if (!existsSync(config.local_root)) {
    mkdirSync(config.local_root, { recursive: true });
  }
  try {
    accessSync(config.local_root, constants.R_OK | constants.W_OK);
  } catch {
    throw new Error(`local_root is not readable/writable: ${config.local_root}`);
  }
}
