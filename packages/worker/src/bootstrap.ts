...
const FALLBACK_CONFIG_TEMPLATE: Record<string, unknown> = {
  '//': 'Worker 配置文件示例（Worker 运行在外网 Windows 主机，本文件内路径均为 Windows 格式）。由 Worker 启动时自动写入共享目录 config/worker.json。worker 启动时只需 --hgfs-root 指向同一共享目录，其余参数自动从这里读取。优先级：命令行参数 > 环境变量 > 本文件 > 内置默认值。',
